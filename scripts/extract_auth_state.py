...
::
    ...
    # every benchmark):
    python scripts/extract_auth_state.py

    # Headed bootstrap (one-time, when Google demands a passkey):
    python scripts/extract_auth_state.py --headed
...
def _save_state(context: BrowserContext, output: Path) -> None:
    ...
    if not _state_has_session(state):
        ...
        raise AuthRefreshError(
            ...
            "couldn't recover. Run `python scripts/extract_auth_state.py --headed` "
    ...
    while time.time() < deadline:
        ...
        if _is_on_challenge(url):
            raise AuthRefreshError(
                ...
                "scripts/extract_auth_state.py --headed` and complete the "
    ...
    *,
    ...
    if not profile_dir.is_dir():
        raise AuthRefreshError(
            ...
            "`python scripts/extract_auth_state.py --headed` once to create it."
        )
