...
# Match run.sh defaults, but force a single worker because this script reruns
# one benchmark task instance.
...
export BROWSERGYM_EXTRA_GOAL_INSTRUCTIONS="If at any point you find yourself signed out of Google (e.g. redirected to https://accounts.google.com/ServiceLogin, https://accounts.google.com/AccountChooser, or shown a 'Sign in' / 'Choose an account' / 'Verify it's you' page), sign back in as agentbenchmark@gmail.com using password Universityofutah. If a 'Verify it's you' interstitial appears with just a Next button, click Next and continue. Then resume the task on the document URL provided above."

refresh_auth() {
    echo "[rerun_sheets2_instance5.sh] Refreshing auth state via extract_auth_state.py..."
    if python "$REPO_ROOT/extract_auth_state.py" \
        --output "$STORAGE_STATE_FILE" \
        --profile-dir "$PROFILE_DIR" \
        --headless \
        --verbose 2>&1 | sed 's/^/  /'; then
        echo "[rerun_sheets2_instance5.sh] Auth refresh OK ($(date '+%H:%M:%S'))."
        return 0
    fi
    echo "[rerun_sheets2_instance5.sh] WARNING: auth refresh failed; benchmark will run on the" >&2
    echo "       existing storage_state.json (possibly stale)." >&2
    return 1
}

RESULTS_SUBDIR="${RESULTS_SUBDIR:-results_axt}"
BENCHMARK="${KNOWS_BENCHMARK:-knows_sheets_2}"
TASKS="${KNOWS_TASKS:-knows.sheets_2_personal_recipe.5}"
SCRIPT="${SCRIPT:-gpt55_axt.py}"

refresh_auth || true

out_dir="$REPO_ROOT/$RESULTS_SUBDIR"
mkdir -p "$out_dir"

echo "[rerun_sheets2_instance5.sh] Running $SCRIPT benchmark=$BENCHMARK tasks=$TASKS -> $out_dir"
AGENTLAB_EXP_ROOT="$out_dir" KNOWS_BENCHMARK="$BENCHMARK" KNOWS_TASKS="$TASKS" \
